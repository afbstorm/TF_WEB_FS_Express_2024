...
const InterventionController = {
    ...
    try {
      ...
      if (!vehiculeId || !typeId || !date_intervention) {
        return res.status(400).send({
          message:
            "L'id du véhicule, du type d'intervention et la date d'intervention sont obligatoires.",
        });
      }
  ...
  updateStatus: async (req, res) => {
    try {
      ...
      if (updated === 0) {
        return res.status(400).send({
          message: `L'intervetion avec l'id : ${id} n'existe pas.`,
        });
      }
