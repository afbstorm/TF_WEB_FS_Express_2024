  ...
  delete: async (req, res) => {
    try {
      ...
      if (interventions.length > 0) {
        return res.status(400).send({
          message:
            "Impossible de supprimer le véhicule car des interventions y sont associées.",
        });
      }

      const deleted = await Vehicule.destroy({ where: { VehiculeID: id } });
  ...
  findByType: async (req, res) => {
    try {
      ...
      const vehiculesByType = vehicules.reduce((acc, vehicule) => {
        ...
        // Vérification de l'existence dans l'acc du type de véhicule sur lequel on itère
        // S'il n'existe pas, on l'initialise en un tableau vide
        if (!acc[type]) {
          acc[type] = [];
        }

        // Ajoutes le véhicule sur lequel on itère dans le tableau correspondant à son type si la clé existe dans l'acc
